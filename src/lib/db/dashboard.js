...
export function setCurrentUser(name) {
  if (name) localStorage.setItem(USER_KEY, name.trim())
...
export async function getVisitedCustomers(salesman) {
  ...
  const [visitsRes, loginsRes, responsesRes] = await Promise.all([
    supabase
      ...
      .select(`
        ...
        created_at,
        customers (
          customer_id, name, mobile, shop_name, owner_name, area, landmark, stage, created_at
        )
      `)
      .eq('salesman_id', salesman)
      .eq('event_type', 'visit_done')
  ...
  return deduped.map(e => {
    ...
    return {
      ...
      customerId:        cid,
      customerCreatedAt: e.customers?.created_at || null,
...
// ── Assigned leads (role-based dashboard) ────────────────────────────────────

/**
 * getAssignedLeads(profileId)
 * profileId = UUID  → only customers assigned to that profile
 * profileId = null  → all customers (admin)
 * Returns same shape as getVisitedCustomers rows.
 */
export async function getAssignedLeads(profileId) {
  const todayStart = startOfToday()
  ...
  const [visitRes, painRes, roiRes, responseRes, loginRes, reminderRes] = await Promise.all([
    supabase.from('events').select('customer_id, event_id, data, created_at')
      ...
      .order('created_at', { ascending: false }),
    supabase.from('events').select('customer_id, data')
      .in('customer_id', customerIds).eq('event_type', 'pain_identified')
      .order('created_at', { ascending: false }),
    supabase.from('events').select('customer_id, data')
      .in('customer_id', customerIds).eq('event_type', 'roi_shown')
      .order('created_at', { ascending: false }),
    supabase.from('events').select('customer_id, data, created_at')
      .in('customer_id', customerIds).eq('event_type', 'customer_response')
      .order('created_at', { ascending: false }),
...
export async function markCustomerFileLogin(salesman, visitEventId) {
  // visitEventId may be an actual event_id or a customer_id (for leads without a visit event)
  ...
  const customerId = visitEvent ? visitEvent.customer_id : visitEventId
