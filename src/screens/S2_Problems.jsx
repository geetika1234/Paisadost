          ...
          return (
            <div key={p.tag}>
              ...
              >
                <div className="flex items-center">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className={`font-bold text-base ${active ? 'text-brand-600' : 'text-slate-800'}`}>
                        {p.title}
                      </p>
                  ...
                  {hasSubs && (
                    <button
                      ...
                      className={`px-3 py-4 text-sm border-l transition-all
                        ${active ? 'border-brand-200 text-brand-400' : 'border-slate-200 text-slate-400'}`}
                    >
                      {isExpanded ? '▲' : '▼'}
                    </button>
                  )}
                ...
                {/* Sub-problems */}
                {hasSubs && isExpanded && (
                  <div className={`border-t space-y-0 ${active ? 'border-brand-200' : 'border-slate-200'}`}>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all
                            ...
                            ${subActive ? 'bg-brand-100' : 'bg-white/60'}`}
                        >
                          {/* Indent indicator */}
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <div className="w-3 h-px bg-slate-300" />
                            <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0
                              ${subActive ? 'bg-brand-600 border-brand-600' : 'border-slate-300 bg-white'}`}>
                              {subActive && <span className="text-white text-[9px] font-bold">✓</span>}
                            </div>
                          </div>
                          <span className={`text-xs font-medium flex-1 ${subActive ? 'text-brand-700' : 'text-slate-600'}`}>
                            {sp.label}
                          </span>
                          {subActive && (
                            <span className="text-[10px] font-bold text-brand-500 bg-brand-100 border border-brand-200 rounded-full px-2 py-0.5 flex-shrink-0">
                              Sub
                            </span>
                          )}
                        </button>
