...
export default function S_QuickCreate() {
  const { closeQuickCreate, activateCustomer, profile } = useApp()

  const [form, setForm] = useState({ shopName: '', city: '', area: '', ownerName: '', mobile: '' })
  const [saving,      setSaving]      = useState(false)
  const [error,       setError]       = useState(null)
  const [mobileError, setMobileError] = useState(null)
  const [duplicateId, setDuplicateId] = useState(null)   // customer_id when mobile already exists
  ...
  async function handleOpenExisting() {
    ...
    setOpening(true)
    try {
      const { customer, events } = await getCustomerFull(duplicateId)
      const latestVisit = (events || []).find(e => e.event_type === 'visit_done')
      activateCustomer({
        id:           customer.customer_id,
        visitEventId: latestVisit?.event_id || null,
        shopName:     customer.shop_name    || '',
        ownerName:    customer.owner_name   || '',
        mobile:       customer.mobile       || '',
        city:         customer.area         || '',
      ...
      setError(err.message || 'Customer kholne mein error aayi.')
    } finally {
      setOpening(false)
    }
  }
  ...
  return (
    <div className="phone-shell flex flex-col bg-slate-100" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 pb-32 space-y-3">
        ...
        {FIELDS.map(f => {
          const hasMobErr = f.key === 'mobile' && !!mobileError
          return (
            <div key={f.key}>
              <div className={`bg-white rounded-2xl border px-4 py-3 shadow-sm ${hasMobErr ? 'border-red-400' : 'border-slate-200'}`}>
                <label className={`text-xs font-bold uppercase tracking-wider ${hasMobErr ? 'text-red-500' : 'text-slate-500'}`}>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    set(f.key, v)
                    if (f.key === 'mobile') { setMobileError(null); setDuplicateId(null) }
                  }}
                  className="w-full mt-1 text-base font-semibold text-slate-800 outline-none placeholder:text-slate-300"
                />
              </div>

              {/* Mobile duplicate error + open existing prompt */}
              {hasMobErr && (
                <div className="mt-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3 space-y-2">
                  <p className="text-xs font-semibold text-red-600">⚠️ {mobileError}</p>
                  {duplicateId && (
                    <button
                      onClick={handleOpenExisting}
                      disabled={opening}
                      className="w-full py-2.5 rounded-xl bg-brand-600 text-white text-xs font-bold active:scale-95 transition-all disabled:opacity-50"
                    >
                      {opening ? 'Kholne mein...' : '👤 Open Existing Customer'}
                    </button>
                  )}
                </div>
              )}
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-white border-t border-slate-200 px-4 py-4 z-30">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            onClick={handleSave}
            className="flex-1 py-3.5 rounded-xl bg-brand-600 text-white text-sm font-bold disabled:opacity-40 active:scale-95 transition-all shadow-lg shadow-brand-200"
          >
